...
const DetailPost = () => {
  ...
  const commentOfPost = useSelector(
    (state) => state.comment.commentpost.commentOfPost
  );
  ...
  const handleLike = async () => {
    ...
    await toast.promise(updatePost({ user: currentUser._id }, post._id), {
      loading: "Đang tải...",
      success: noti,
  ...
  const handleSave = async () => {
    ...
    await toast.promise(savePost({ user: currentUser._id }, post._id), {
      loading: "Đang tải...",
      success: mess,
      error: "lỗi đường truyền",
    });
    ...
    if (showComment && post) {
      ...
      if (redirectNoti?.action === "interComment") {
        ...
        if (redirectNoti?.action === "replyComment") {
          handleScrollElement(
            redirectNoti.reaction,
            redirectNoti.sender_comment
          );
        } else {
            ...
            <div className="detailpost__header__infor">
              ...
              {post && (
                <div className="detailpost__header__icon">
                  <div
                    className="detailpost__header__save"
                    onClick={handleSave}
                  >
              ...
              <div className="detailpost__inter__relatedPost">
                {relatedPost?.length > 0 ? (
                  relatedPost?.map((post, index) => (
                    <RelatedPost key={index} post={post} setPost={setPost} />
                  ))
                ) : (
            ...
            {post ? (
              <div className="detailpost__footer__inter">
                <div onClick={handleLike}>
                  {post?.like_user.filter((e) => e._id === currentUser._id)
                    .length > 0 ? (
      ...
      {post && (
        <div className="detailpost__comment">
          ...
          <div id="comment" className="detailpost__idea">
            <div
              className="detailpost__idea__closeTab"
              onClick={handleOffComment}
            >
            ...
            <div className="detailpost__idea__container">
              <div className="detailpost__idea__commentCount">
                ...
                <p>
                  (Vui lòng không spam các nội dung không lành mạnh hoặc gây
                  war!)
                </p>
              </div>
