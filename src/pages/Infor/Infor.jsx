...
const Infor = ({ save }) => {
  ...
  const uploadImage = async (base64encodedImage) => {
    try {
      const res = await publicRequest.post(
        `/v1/user/upload/user/${currentUser._id}`,
  ...
  const receiveData = (data) => {
    if (data.delete) {
      toast.promise(deleteUser(dispatch, data.user, navigate), {
        loading: "Đang tải...",
      ...
      <section className="infor">
        <div className="infor__container">
          <div className="infor__simple">
            {user ? (
              <div
                className="infor__simple__bg"
                style={{
                  backgroundImage: `url(https://static.vecteezy.com/system/resources/previews/000/549/810/original/vector-abstract-technology-background-technology-digital-world-of-business-information-futuristic-blue-virtual-graphic-interface.jpg)`,
                }}
              ></div>
            ) : (
              <Skeleton
                  ...
                  <div className="infor__detail__private__form">
                    ...
                    <div className="infor__detail__private__item">
                      ...
                      <span>
                        {user.posts.length === 0
                          ? "chưa có bài viết"
                          : `Đã đóng góp ${user.posts.length} bài viết`}
                      </span>
                    </div>
            ...
            <div className="infor__detail__product">
              {user ? (
                <div className="infor__detail__product__option">
                  <h1>bài viết</h1>
                  <div
                    className="infor__detail__product__selection"
                    onClick={(e) => handleSelect(e)}
                  >
                    <div
                      id="all"
                      className={`infor__detail__product__item ${
                        save ? "" : "selected"
                      }`}
                    >
                    ...
                    >
                      <CollectionsBookmarkIcon
                        sx={{ fontSize: 20, color: amber[400] }}
                      />
              ...
              {user ? (
                <div className="infor__detail__product__allPost">
                  {save ? <h1>Bài viết đã lưu</h1> : <h1>Tất cả bài viết</h1>}
                  {post.length > 0 ? (
                    <Grid
