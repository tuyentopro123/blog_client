...
const NewPost = () => {
        ...
        const uploadImagePost = async () => {
          try {
            const res = await publicRequest.post(
              `/v1/post/upload/post/${currentUser._id}`,
              { data: reader.result }
            );
            quillObj
              .getEditor()
              .insertEmbed(range.index, "image", res.data.url);
          } catch (err) {
            console.error(err);
          }
        };
  ...
  const uploadImage = async (base64encodedImage) => {
    try {
      const res = await publicRequest.post(
        `/v1/post/upload/thumb/${currentUser._id}`,
          ...
          <div className="new__body">
            <div className="new__body__thumbnail">
              ...
              <label htmlFor="imgPost">
                <div style={{ position: "relative" }}>
                  ...
                  {isLoading && (
                    <div className="loading_thumbnail">
                      <CircularProgress
            ...
            <div className="new__body__blog">
              ...
              <div className="new__body__blog__category">
                <div
                  ref={selected}
                  className="new__body__blog__category__select"
                >
                  <select onChange={(e) => handleField(e)} disabled={isLoading}>
                ...
                {post.category.length > 0 && (
                  <Stack
                    className="new__body__blog__category__input"
                    direction="row"
                    spacing={1}
                  >
                    {post.category.map((caterory, index) => (
                      <Chip
                        ...
                        key={index}
                        label={caterory}
                        onDelete={() => handleDelete(caterory)}
                      />
                    ))}
                  </Stack>
                )}
