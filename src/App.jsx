    ...
    <div className="App">
      ...
      <BrowserRouter>
        ...
        <Routes>
          ...
          <Route
            path="/infor/:id"
            element={user ? <Infor save={false} /> : <Login />}
          />
          <Route
            path="/infor/save/:id"
            element={user ? <Infor save={true} /> : <Login />}
          />
          <Route path="/add" element={user ? <NewPost /> : <Login />} />
          <Route path="/setting" element={<Setting />} />
          <Route
            path="/post/:slug"
            element={user ? <DetailPost /> : <Login />}
          />
          <Route
            path="/admin"
            ...
            <div className="footer__categories__content">
              ...
              >
                {brands.slice(0, 6).map((item, index) => (
                  <li key={index}>
                    <a href={`/program?category=${item}`}>{item}</a>
                  </li>
                ))}
                {brands.slice(6, 11).map((item, index) => (
                  <li key={index}>
                    <a href={`/life?category=${item}`}>{item}</a>
                  </li>
                ))}
              </ul>
            </div>
...
function SpeedDialTooltip({ user }) {
  ...
  return (
    ...
    >
      {(user?.isAdmin ? actions : actions.slice(0, 2)).map((action) => (
        <SpeedDialAction
          key={action.name}
          icon={action.icon}
